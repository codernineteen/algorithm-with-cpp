...
pair<int, PATH> bfs(int sr, int sc, int tr, int tc) {
	...
	while (!nexts.empty()) {
		...
		for (auto& dir : dirs) {
			...
			if (is_out_range(nr, nc)) {
				if (dir[0] == 1) nr = 1; // 하
				else if (dir[0] == -1) nr = N; // 상
				else if (dir[1] == 1) nc = 1; // 우
				else  nc = M; // 좌
			}
...
bool razer_atk(int sr, int sc, int tr, int tc) {
	...
	for (auto& dir : dirs) {
		...
		pair<int, PATH> res = bfs(nr, nc, tr, tc);
...
pair<int, int> get_target(int ar, int ac)
{
	pair<int, int> vict;
	int power = 1;
	int history = 0;
...
void fix_top(int ar, int ac)
{
	for (int i = 1; i <= N; i++) {
		for (int j = 1; j <= M; j++) {
			if (board[i][j] > 0 && !participate[i][j]) board[i][j] += 1; // 공격력 회복
			if (board[i][j] > 0 && ar != i && ac != j) oldest[i][j]++; // 휴식 추가
		}
	}
}
...
void simulate()
{
	...
	// 2. 공격 대상 선정
	auto target_coor = get_target(atk_coor.R, atk_coor.C);
